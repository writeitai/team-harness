...
Hard provider limits reported by worker JSONL are recorded separately under
`rate_limited_families`. Each entry contains `family`, the effective `model`
when known, `tripped_at`, `resets_at`, and `reason`. These records are additive
audit history; existing per-turn usage fields are unchanged.
