...
| `grok`        | `grok`                      | `--model` (default `grok-4.5`)        |
...
- **Grok** runs headless with `--always-approve`, `--output-format streaming-json`, and `--no-auto-update`. Session ids are captured from the final NDJSON `end` event (`sessionId`, camelCase) and resume uses `--resume <id>`. Auth is external: set `XAI_API_KEY` or run `grok login`. Set `GROK_HOME` when CI needs an isolated Grok home; `GROK_DISABLE_AUTOUPDATER=1` is an optional environment-policy fallback to `--no-auto-update` and is not injected by the built-in template. Do not put `--session-id` in `shared_flags` for this worker — combining `-s` with `--resume` forks rather than resumes. A crash before the `end` event may leave no captured session id. Reasoning effort uses `--reasoning-effort`. If you previously defined a custom `[agents.grok]` for an unrelated binary, rename it or explicitly clear/replace inherited fields after upgrading.
- **Antigravity** runs use `agy --print` so the worker runs as a non-interactive subprocess. Print mode does not emit stream-json, so automatic session capture is not configured — callers that already know a conversation id can still use resume mode (`--conversation <id>`).
