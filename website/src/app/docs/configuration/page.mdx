...
When a finished worker's JSONL reports a terminal 429 or rejected rate-limit
event, team-harness blocks later spawns of that agent-template family until the
provider reset time. If the event has no reset time, the default cooldown is
900 seconds. The coordinator can query `agent_availability`, and the trip is
recorded under `run.json.rate_limited_families`.
